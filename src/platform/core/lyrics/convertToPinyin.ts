...
const convertLyricsToPinyin = async (repository: LyricsRepository) => {
  ...
  try {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (typeof line === 'string') {
        if (!(await hasConvertibleCharacter(line))) convertedLyrics.push([]);
        ...
        for (let j = 0; j < line.length; j++) {
          const word = line[j];
          if (!(await hasConvertibleCharacter(word.text)))
            convertedSyncedWords.push(word.text.trim());
          else {
