...
import getArtistInfoFromLastFM from './lastFm/getArtistInfoFromLastFM';
import { isConnectedToInternet as checkInternetConnection } from './isOnline';
...
const getArtistDataFromSavableArtistData = (repository: NetworkRepository, artist: SavableArtist): Artist => {
  const artworkPaths = repository.getArtistArtworkPath(artist.artworkName);
      ...
      for (const artist of artists) {
        if (artist.name === unparsedSimilarArtist.name) {
          availableArtists.push({
            ...
            artistData: getArtistDataFromSavableArtistData(repository, artist)
          });
...
const getArtistInfoFromNet = async (repository: NetworkRepository, artistId: string): Promise<ArtistInfoFromNet> => {
  logger.debug(
    `Requested artist information related to an artist with id ${artistId} from the internet`
  );
  const artists = repository.getArtists();
    ...
    for (let x = 0; x < artists.length; x += 1) {
      if (artists[x].artistId === artistId) {
        ...
        if (artistArtworks && artistInfo) {
          const artistPalette = await repository.generatePalette(artistArtworks.picture_medium);
          const similarArtists = getSimilarArtistsFromArtistInfo(repository, artistInfo, artists);

          if (!artist.onlineArtworkPaths) {
            artists[x].onlineArtworkPaths = artistArtworks;
            repository.setArtists(artists);
            repository.emitDataUpdate('artists/artworks');
          }
          return {
            artistArtworks,
