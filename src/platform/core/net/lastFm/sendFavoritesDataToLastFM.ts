...
const sendFavoritesDataToLastFM = async (
  ...
  try {
    ...
    if (isSendingLoveEnabled && isConnectedToInternet) {
      const authData = await getLastFmAuthData(repository);
...
export const addAFavoriteToLastFM = (repository: NetworkRepository, title: string, artists: string[] = []) =>
  sendFavoritesDataToLastFM(repository, 'track.love', title, artists);

export const removeAFavoriteFromLastFM = (repository: NetworkRepository, title: string, artists: string[] = []) =>
  sendFavoritesDataToLastFM(repository, 'track.unlove', title, artists);

export default sendFavoritesDataToLastFM;
