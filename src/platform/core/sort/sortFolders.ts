...
const isFolderBlacklisted = (repository: BlacklistRepository, folderPath: string) =>
  repository.getFolderBlacklist().includes(folderPath);
  ...
  if (musicFolders.length > 0) {
    for (const musicFolder of musicFolders) {
      if (musicFolder.subFolders.length > 0) {
        musicFolder.subFolders = sortFolders(repository, musicFolder.subFolders, sortType);
      }
    }
    ...
    if (sortType === 'blacklistedFolders')
      return sortAtoZ(musicFolders.filter((folder) => isFolderBlacklisted(repository, folder.path)));

    if (sortType === 'whitelistedFolders')
      return sortAtoZ(
        musicFolders.filter((folder) => !isFolderBlacklisted(repository, folder.path))
      );
  }
  return musicFolders;
};
