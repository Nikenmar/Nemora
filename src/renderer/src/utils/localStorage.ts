...
const resetLocalStorage = () => {
  try {
    ...
    // The store may not exist yet. checkLocalStorage() runs at store.ts module
    // scope, ABOVE the `store` and `dispatch` declarations, so on a first run,
    // where the storage is empty and this repair path is taken, touching
    // `dispatch` throws from the temporal dead zone. That threw on every fresh
    // install, and because the catch below logged the error as `{}` nobody
    // could see what it was.
    //
    // Nothing is lost by skipping it there: the keys above are already written,
    // and store.ts dispatches the freshly read state immediately after it
    // finishes initialising. The runtime caller, Settings "Clear optional
    // data", is the one that genuinely needs this dispatch, and by then the
    // store exists.
    try {
      dispatch({ type: 'UPDATE_LOCAL_STORAGE', data: LOCAL_STORAGE_DEFAULT_TEMPLATE });
    } catch {
      /* store not initialised yet; store.ts dispatches the same state next */
    }
  } catch (error) {
